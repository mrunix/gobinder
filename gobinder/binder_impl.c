...
/* A helper function to create 'struct binder_io' */
struct binder_io *new_binder_io() {
  struct binder_io *p = (struct binder_io *) malloc(sizeof(struct binder_io));
  memset(p, 0, sizeof(struct binder_io));
  return p;
}

void free_binder_io(struct binder_io *p) {
  free(p);
}
...
struct binder_state* binder_open(size_t mapsize) {
  ...
  if ((ioctl(bs->fd, BINDER_VERSION, &vers) == -1) ||
    ...
    fprintf(stderr, "binder: driver version(%d) differs from user space version(%d)\n", vers.protocol_version, BINDER_CURRENT_PROTOCOL_VERSION);
    goto fail_open;
  }
